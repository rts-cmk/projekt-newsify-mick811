...
import { fetchTopStories, fetchMostPopular } from '../src/hooks/useFetch';
...
describe('fetch helpers', () => {
  ...
  it('fetchTopStories returns parsed top stories response when fetch ok', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce(mockResponse(body)));
  ...
  it('fetchMostPopular deduplicates and filters by category sections', async () => {
    ...
    const articleA = { id: 1, section: 'World', subsection: '', title: 'A', abstract: '', uri: 'uri-a', url: '', asset_id: 1, source: 'New York Times', published_date: '', updated: '', nytdsection: '', adx_keywords: '', column: null, byline: '', type: 'Article', des_facet: [], org_facet: [], per_facet: [], geo_facet: [], media: [], eta_id: 0 };
    const articleB = { id: 2, section: 'Sports', subsection: '', title: 'B', abstract: '', uri: 'uri-b', url: '', asset_id: 2, source: 'New York Times', published_date: '', updated: '', nytdsection: '', adx_keywords: '', column: null, byline: '', type: 'Article', des_facet: [], org_facet: [], per_facet: [], geo_facet: [], media: [], eta_id: 0 };
    const articleADuplicate = { ...articleA };

    const resp1 = { status: 'OK', copyright: '©', num_results: 2, results: [articleA, articleB] };
    const resp2 = { status: 'OK', copyright: '©', num_results: 1, results: [articleADuplicate] };
    const resp3 = { status: 'OK', copyright: '©', num_results: 0, results: [] };
